...
| References — bioinformatics software | Bioinformatics | Listed in back-pocket §8 |
...
Inter-run reproducibility was 100% across all 9 sequencing runs. (Wilson 95% confidence intervals are in the back-pocket package §3.1.1.)
...
The cohort therefore does not contain melioidosis as detected by this assay. (Earlier reports of a preserved *B. pseudomallei* detection in sample `23200430_S6_L001` were based on a substring match in the taxonomy report that conflated genus-level read counts with species-level counts. The actual species-level *B. pseudomallei* count in that sample is 8 reads, below the run NTC's 10–51 reads.)
...
(Six additional limitations — pilot sample size, species-level resolution limits, filter dataset-dependence, no clinical-outcome correlation in the bioinformatics package, no anaerobic culture comparator, and the practical *B. pseudomallei* detection floor — are documented in the back-pocket package §5 and can be added if your target journal expects a comprehensive limitations section.)

---

## §H. Where to find the rest

The full back-pocket package is checked in to the repository at [`docs/manuscript/`](..) (everything in the parent folder of this handoff). The most useful files if you need more detail than the slim handoff:

- **Full integrated manuscript draft (~9,000 words)**: [`docs/manuscript/MANUSCRIPT-PACKAGE-FINAL.md`](../MANUSCRIPT-PACKAGE-FINAL.md) — contains the comprehensive Methods (with software citations, compute resources, threshold tables), Results (with Wilson CIs, organism-specific sensitivity tables, full Sankey discussion), Discussion (with hypothesis-evaluation tables), Limitations (all 10), and References. Use this as a fact-check and depth reference.
- **Condensed JCM-style draft (~3,900 words)**: [`docs/manuscript/MANUSCRIPT-CONDENSED-DRAFT.md`](../MANUSCRIPT-CONDENSED-DRAFT.md).
- **Per-sample tables (Excel + markdown)**: [`docs/manuscript/appendices/`](../appendices/) — `APPENDICES.xlsx` (4 sheets: Validation-detections, Validation-sample-summary, Study-detections, Study-sample-summary), `APPENDIX-VALIDATION-PANEL.md`, `APPENDIX-STUDY-SAMPLES.md`.
- **V4 decontamination filter audit**: [`docs/manuscript/appendices/DECONTAMINATION-FILTER-REPORT-V4.txt`](../appendices/DECONTAMINATION-FILTER-REPORT-V4.txt).
- **Filter script (executable)**: [`docs/manuscript/scripts/afi_decontamination_filter_v4.py`](../scripts/afi_decontamination_filter_v4.py) — the V4 filter source. Open this if you need the full Tier-A / Tier-B / Tier-1 / Tier-2 genus lists.
- **Appendix generator**: [`docs/manuscript/scripts/generate_appendices.py`](../scripts/generate_appendices.py) — regenerates the appendix tables from raw pipeline outputs.
- **Figure 1 (Sankey, V4 filter impact)**: [`docs/manuscript/figures/figure_a_sankey.html`](../figures/figure_a_sankey.html) (interactive) and [`docs/manuscript/figures/figure_a_sankey.png`](../figures/figure_a_sankey.png) (for print). Caption suggestion: "Aggregated read-volume flow from each detected genus (left) to each V4 filter outcome (right) across the 86-sample study cohort."
- **Thai walkthrough (slim version)**: [`AFI-TEAM-HANDOFF-TH.md`](AFI-TEAM-HANDOFF-TH.md) — for wet-lab team members who would prefer Thai. The earlier longer Thai walkthrough is at [`docs/manuscript/MANUSCRIPT-WALKTHROUGH-THAI.md`](../MANUSCRIPT-WALKTHROUGH-THAI.md) (457 lines).
- **Framing, citation starters, EPI question prompts, skeleton outlines**: [`LEADS-AND-FRAMING.md`](LEADS-AND-FRAMING.md) — for the sections the wet-lab + EPI team owns.
- **Pipeline source code**: https://github.com/PHemarajata/afi_terra.

If you find a number in this document that conflicts with a number in the back-pocket, the back-pocket file is the source of truth — please flag the conflict to bioinformatics before May 31.
